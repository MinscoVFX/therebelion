...
function parsePubkey(label: string, v: string): PublicKey {
  try {
    ...
    throw new Error(`${label} is not a valid pubkey`);
  }
}
...
async function main() {
  ...
  const client: any = new DynamicBondingCurveClient(
    connection as any,
    wallet.payer as any
  );

  // Resolve LP mint from pool (supports multiple SDK versions)
  let lpMint: PublicKey | null = null;
  const st = (await client
    .getPoolState?.(pool)
    .catch(() => null)) as { lpMint?: string } | null;
  if (st?.lpMint) {
    lpMint = new PublicKey(st.lpMint);
  } else {
    const acc = (await client.getPool?.(pool).catch(() => null)) as
      | { lpMint?: string }
      | null;
  ...
  if (!lpMint) {
    throw new Error(
      "Could not resolve LP mint for this pool (not a Meteora DBC pool?)"
    );
  }
  ...
  const amount =
    amountArg === "MAX"
      ...
      : (() => {
          if (!/^\d+$/.test(amountArg))
            throw new Error("amount must be integer raw units or MAX");
  ...
  if (withdrawIx) {
    ...
    signature = await connection.sendRawTransaction(tx.serialize(), {
      skipPreflight: true, // speed path; you can set false if you prefer
    });
    ...
    typeof client.withdraw === "function" ||
    typeof client.removeLiquidity === "function"
  ) {
    const call =
      typeof client.withdraw === "function"
    ...
    if (typeof res === "string") {
      ...
      if (preIxs.length)
        throw new Error(
          "Priority fee can’t be appended to SDK v0 tx; omit --priority or update SDK."
        );
      ...
      throw new Error(
        "Unrecognized SDK return; upgrade @meteora-ag/dynamic-bonding-curve-sdk."
      );
    }
  } else {
    throw new Error(
      "SDK version lacks withdraw helpers. Upgrade @meteora-ag/dynamic-bonding-curve-sdk."
    );
  }
