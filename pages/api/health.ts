...
export default async function handler(_req: NextApiRequest, res: NextApiResponse) {
  try {
    // Attempt env parse but do not make non-RPC schema issues fatal for basic health.
    // Use a loose object fallback; avoid 'any' for lint compliance.
    let env: Record<string, unknown> | undefined = undefined;
    try {
      env = getEnv();
    } catch {
      // Swallow schema errors here; we only want to fail hard if absolutely no RPC is configured.
    }
    ...
    const cluster =
      ((ver as Record<string, unknown>)['solana-core'] as string) ||
      ((ver as Record<string, unknown>).solana_core as string) ||
      'unknown';
    ...
    res.status(fatal ? 500 : 200).json({
      ...
      env: {
        ...
        dbcSelector: env?.DBC_CLAIM_FEE_DISCRIMINATOR
          ? 'disc'
          : env?.DBC_CLAIM_FEE_INSTRUCTION_NAME || 'auto',
