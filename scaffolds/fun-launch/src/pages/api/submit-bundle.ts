...
function parsePubkey(label: string, value?: string) {
  ...
  try { return new PublicKey(value.trim()); } catch { throw new Error(`${label} is not a valid base58 pubkey`); }
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const referralTA = referralTokenAccount ? parsePubkey("referralTokenAccount", referralTokenAccount) : null;
