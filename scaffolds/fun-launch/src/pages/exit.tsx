  ...
  return (
    <div className="border rounded-lg p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[11px] uppercase tracking-wide text-gray-600">Simulate First</label>
          <input
            type="checkbox"
...
const BatchExitPanel: React.FC = () => {
  ...
  const runBatch = useCallback(async (): Promise<void> => {
    ...
    for (let i = 0; i < eligiblePools.length; i++) {
      ...
      try {
        const sig = await exit({ dbcPoolKeys: { pool: p.pool, feeVault: p.feeVault }, priorityMicros: priority, simulateFirst, slippageBps });
    ...
    setRunning(false);
  }, [publicKey, eligiblePools, exit, priority, simulateFirst, slippageBps, running]);
  ...
  return (
    <div className="border rounded-lg p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[11px] uppercase tracking-wide text-gray-600">Simulate First</label>
          <input type="checkbox" checked={simulateFirst} onChange={e => setSimulateFirst(e.target.checked)} />
        </div>
        <div className="flex items-end">
          <button
