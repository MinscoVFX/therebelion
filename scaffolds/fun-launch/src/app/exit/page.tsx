    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        {pools.length > 0 && (
          <div className="space-y-2">
            {pools.map(pool => (
              <label key={pool.id} className="flex items-center p-3 border rounded-md hover:bg-gray-50 cursor-pointer">
                <input
                  type="radio"
                  name="selectedPool"
                  value={pool.id}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-500">
                    LP Amount: {pool.lpAmount.toString()}
                  </div>
                </div>
              </label>
            ))}
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <input
              ...
              onChange={(e) => setPrefs(prev => ({ ...prev, priorityMicros: Number(e.target.value) }))}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setPrefs(prev => ({ ...prev, slippageBps: Number(e.target.value) }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        ...
        <div className="space-y-3 mb-6">
          ...
          <label className="flex items-center">
            <input
              ...
              className="mr-2"
            />
            <span className="text-sm">Fast mode (skip simulation, processed-first confirmation)</span>
          </label>
        </div>
        ...
        <div className="flex gap-4">
          ...
          {exitState.status !== 'idle' && exitState.status !== 'success' && exitState.status !== 'error' && (
            <button
              onClick={abort}
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className={`font-medium ${
                exitState.status === 'success' ? 'text-green-600' :
                exitState.status === 'error' ? 'text-red-600' :
                'text-blue-600'
              }`}>
                {exitState.status}
              </span>
            ...
            {exitState.simulation && (
              <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                <p className="text-blue-800 font-medium mb-2">
                  Simulation: {exitState.simulation.logs.length} logs, {exitState.simulation.unitsConsumed} CU
                </p>
                {exitState.simulation.logs.slice(0, 5).map((log, i) => (
                  <p key={i} className="text-xs text-blue-700 font-mono">{log}</p>
                ))}
              </div>
            )}
          </div>
