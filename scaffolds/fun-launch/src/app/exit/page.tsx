...
export default function ExitPage() {
  ...
  const [prefs, setPrefs] = useState<ExitPreferences>({ priorityMicros: 750_000, slippageBps: 10_000, simulateFirst: true, fastMode: false });

  // restore prefs
  useEffect(() => {
    if (typeof window === 'undefined') return;
  try { const saved = window.localStorage.getItem('dbc-exit-prefs'); if (saved) setPrefs(p=>({...p,...JSON.parse(saved)})); } catch { /* ignore */ }
  }, []);
  // persist prefs
  useEffect(() => {
    if (typeof window === 'undefined') return;
  try { window.localStorage.setItem('dbc-exit-prefs', JSON.stringify(prefs)); } catch { /* ignore */ }
  }, [prefs]);

  // Auto-select the pool with the largest LP amount (heuristic for "latest/migrated")
  const selectedPool = pools.reduce((acc,p)=> !acc || p.lpAmount > acc.lpAmount ? p : acc, undefined as any);
  const canExit = connected && selectedPool && exitState.status === 'idle';

  async function handleExit() {
    if (!selectedPool) { toast.error('No DBC position detected to exit'); return; }
    try {
      const dbcPoolKeys: DbcPoolKeys = { pool: selectedPool.pool, feeVault: selectedPool.feeVault };
      const sig = await exit({ dbcPoolKeys, action: 'claim_and_withdraw', priorityMicros: prefs.priorityMicros, slippageBps: prefs.slippageBps, simulateFirst: prefs.simulateFirst, fastMode: prefs.fastMode, computeUnitLimit: prefs.computeUnitLimit });
      if (sig) toast.success(`Exited position: ${sig.slice(0,8)}...`);
    } catch (e:any) { toast.error(`Exit failed: ${e?.message||e}`); }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 text-neutral-100 bg-neutral-900 min-h-screen" role="main">
      ...
      <p className="text-neutral-400 mb-6 text-sm">Automatically claims all fees and withdraws 100% liquidity from your most significant detected DBC position in a single transaction.</p>
      ...
      <div className="space-y-2">
        {selectedPool && (
          <div className="p-4 border border-neutral-700 rounded-md bg-neutral-850">
            ...
            <div className="mt-2 text-xs text-neutral-400">Pool: {selectedPool.pool.slice(0,8)}... FeeVault: {selectedPool.feeVault.slice(0,8)}... LP: {selectedPool.lpAmount.toString()}</div>
          </div>
        )}
        {!selectedPool && !loading && <div className="text-neutral-500 text-sm">No DBC positions detected.</div>}
      </div>
