...
import { scanDbcPositionsUltraSafe, discoverMigratedDbcPoolsViaNfts, discoverMigratedDbcPoolsViaMetadata } from '@/server/dbc-adapter';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const nftPoolSet = [...new Map([...nftPools, ...metaPools].map(p => [p.toBase58(), p])).keys()];
