...
if (process.env.ALLOWED_DBC_PROGRAM_IDS) {
  const entries = process.env.ALLOWED_DBC_PROGRAM_IDS.split(',').map(s => s.trim()).filter(Boolean);
...
if (errors.length) {
  log('error', `FAILED (${errors.length} errors, ${warnings.length} warnings) commit=${commit||'n/a'}`);
  process.exit(1);
} else {
